...
export default function CollectionComponent({
  ...
  const [addOrRemoveRecipeFromCollection] = useMutation(ADD_OR_REMOVE_RECIPE_FORM_COLLECTION);
  const user = useUser();
  const { changeRecipeWithinCollection, activeRecipeId, singleRecipeWithinCollections } = useAppSelector(
  ...
  const { referenceOfRecipeUpdateFunc } = useAppSelector((state) => state?.recipe);
  const { bulkRecipeIdsForAddedInCollection } = useAppSelector((state) => state?.collections);
  const [shareCollection, { loading: shareCollectionLoading }] = useMutation(CREATE_SHARE_COLLECTION_LINK);
  ...
  const generateShareLink = async (isGlobalShare: boolean = true) => {
    try {
      ...
      const generatedLink = `${
        ...
        isGlobalShare ? "token=" + data.createShareCollectionLink : "collectionId=" + collectionInfo?.id
      }`;
      setLink(link);
      ...
      await addOrRemoveRecipeFromCollection({
        variables: {
          data: {
            ...
            recipes: activeRecipeId ? [activeRecipeId] : bulkRecipeIdsForAddedInCollection,
  ...
  const handleChange = async (e, collectionId) => {
    ...
    if (e?.target?.checked) {
      ...
      setCollectionHasRecipe((pre) => [...pre?.filter((id) => id !== collectionId)]);
    }
  };
  ...
  return (
    <div>
      {changeRecipeWithinCollection ? null : (
        <Fragment>
          <SingleCollection name="My Recipes" slug="my-recipes" collectionRoute="recipeCollection" />
          <SingleCollection
            name="Shared With Me"
