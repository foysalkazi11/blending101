...
import { setIsActiveBlogForCollection, updateLastModifiedBlogCollection } from "../../redux/slices/blogSlice";
...
const useToAddBlogCollection = () => {
  ...
  let timeOut;
  const [addToBlogCollection] = useMutation(ADD_TO_LAST_MODIFIED_BLOG_COLLECTION, { fetchPolicy: "network-only" });
  const handleToAddBlogCollection = async (
    ...
    try {
      ...
      dispatch(updateLastModifiedBlogCollection(addToLastModifiedBlogCollection));
      dispatch(
      ...
      // });
      blogFieldUpdateFunction(blogId, {
        blogCollections: [addToLastModifiedBlogCollection?._id],
      });
