...
function GraphDetailContainer({ statistics, activities }: GraphDetailContainerInterface) {
  const chartData = useMemo(() => {
    ...
    Object.values(categories).forEach((category: any) => {
      ...
      category.description = `${((category.value * 100) / total).toFixed(1)}% (${category.value})`;
      data.push(category);
    });
  ...
  return (
    <div className={styles.challenge_circle_graph_data}>
      <div className={styles.challenge_circle_streak}>
        <p className={styles.challenge_circle_no_of_days}>{statistics?.currentStreak || 0} Days</p>
        <p className={styles.challenge_circle_paragraph}>Current Streak</p>
      </div>
      <div className={styles.challenge_circle_graph_box}>
        <HSBar height={"40px"} id={styles.challenge_circle_new_id} data={chartData} />
      </div>
      <div className={styles.challenge_circle_streak}>
        <p className={styles.challenge_circle_no_of_days}>{statistics?.longestStreak || 0} Days</p>
