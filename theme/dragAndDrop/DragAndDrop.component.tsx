...
const DragAndDrop = () => {
	...
	const handleSubmit = e =>{
		e.preventDefault()
		if (inputVlaue) {
			setNewIngredientsList(pre =>[...pre,{label:inputVlaue,id:Date.now()}])
		}
	}
	...
	const deleteItem =(id:number)=>{
		...
		/* @ts-ignore */
		const filterData = newIngredientsList?.filter(item => item?.id !== Number(id))
	...
	const onDragEnd = result => {
		...
		if (source.droppableId !== "droppable") {
			if (destination.droppableId === "droppable") {
				 const id = existingIngredientsList[parseInt(source?.droppableId)][source?.index]?.id;
				 const item = findItem(id)
				 
				if (!item) {
					setNewIngredientsList(state =>
						copy(existingIngredientsList[parseInt(source.droppableId)], state, source, destination)
					  );
				}else{
					return
				}
    ...
    return (
		<>
        	...
        	<div className="compare-down-sec">
                            ...
                            <SectionTitleWithIcon title="Ingredients" icon="/icons/right-blender.svg" />
								<div className="add-recipe-form  ingredients">
									...
									{(provided, snapshot) =>(
										...
										>
										{newIngredientsList?.map((item,index)=>(
											/* @ts-ignore */
											<Draggable key={item?.id} draggableId={`${item?.id}`} index={index} >
												{(provided, snapshot) =>(
													...
													<RecipeItem item= {item} plusIcon={false} deleteIcon={true} handleDelete={deleteItem}/>
					...
					<div className="formulate-compare-wrp" style={{display:"flex"}}>
						...
						existingIngredientsList?.map((list,index)=>{
							return(
								<div className="brdr-wrp-compareitems" style={{flexGrow:1}} key={index}>
                                    <SectionTitleWithIcon title="Ingredients" icon="/icons/right-blender.svg" />
									
									<ul className="make-recipe-listing">
									
									<Copyable items={list} addItem={addItem} droppableId={`${index}`} />
